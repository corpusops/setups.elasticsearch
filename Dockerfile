# docker build --squash -t corpusops/elasticsearch:5.4.0 . -f Dockerfile --build-arg=SKIP_COPS_UPDATE=y --build-arg=COPS_IMG=corpusops/elasticsearch:5.4.0
#FROM corpusops/elasticsearch
...
ARG COPS_IMG=
ARG COPS_PROJECT_TYPE=elasticsearch
...
RUN bash -c '\
  ...
  && rsync -av /srv/projects/elasticsearch/data/ /srv/projects/elasticsearch/data.sample/'
